...
public class SOTAwoCP {
    ...
    public SOTAwoCP(JSInstance data, long timeLimit, int ub) {
        this.data = data;
...
//        System.out.println(Xs.getFirst());
    ...
    private State transition(State state, BitSet done, int decision){
        ...
        newState.lb = lowerBound(newState, done);
    ...
    private int lowerBound(State state, BitSet done){
        int bound = 0;
        ...
        for (int i = 0; i < data.getnMachines(); i++) {
            for (int j = 0; j < data.getnJobs(); j++) {
                ...
                if (done.get(j*data.getnMachines()+i)) {
